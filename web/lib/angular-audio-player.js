    ...
    AudioPlayer.prototype = {
      ...
      next: function (autoplay) {
        ...
        if (self.currentTrack && self.currentTrack < self.tracks) {
         ...
         // On change le titre de la chanson
         self._scope.$parent.currentSong = self._playlist[self.currentTrack].title;
        }
      },
      prev: function (autoplay) {
        ...
        if (self.currentTrack && self.currentTrack - 1) {
          ...
          // On change le titre de la chanson
           self._scope.$parent.currentSong = self._playlist[self.currentTrack -2].title;
        }
      },
      _addAudioList: function (audioList) {
        ...
        if (angular.isArray(audioList)) {
          angular.forEach(audioList, function (singleElement, index) {
            var sourceElem = angular.element($interpolate('<source src="{{ src }}" type="{{ format }}" media="{{ media }}">')(singleElement));
            self._element.append(sourceElem);
          });
        } else if (angular.isObject(audioList)) {
          var sourceElem = angular.element($interpolate('<source src="{{ src }}" type="{{ format }}" media="{{ media }}">')(audioList));
          self._element.append(sourceElem);
        }
      },
    ...
    return {
      ...
      link: function (scope, element, attrs, ctrl) {
        ...
        angular.forEach(sourceElements, function (sourceElement, index) {
          audioElement.push({ src: sourceElement.src, type: sourceElement.format, media: sourceElement.media });
        });
